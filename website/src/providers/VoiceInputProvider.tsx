...
export function usePersistentVoiceInput(
  ...
  if (!context) throw new Error('usePersistentVoiceInput must be used within VoiceInputProvider')

  const { configure, unregister } = context
  const handlersRef = useRef<VoiceHandlers>({ onText })
  handlersRef.current.onText = onText
  handlersRef.current.onPartial = options.onPartial
  handlersRef.current.onEndpoint = options.onEndpoint
  useEffect(() => {
    configure(options, handlersRef.current)
    return () => unregister(handlersRef.current)
  }, [configure, options, unregister])
